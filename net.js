...
if (!v4.primaryAddress) {
    ...
    if ($network['cellular-data']) {
        ...
        if (carrierId && radio) {
            cellularInfo = carrierNames[carrierId] ?
                ...
                '行动数据 | ' + radioGeneration[radio] + ' - ' + radio;
        }
    }
    $httpClient.get('http://ip-api.com/json', function (error, response, data) {
        ...
        $done({
            title: wifi.ssid ? wifi.ssid + `    IPv4 : ${v4.primaryAddress}` : cellularInfo + `    IPv4 : ${v4.primaryAddress}`,
            content:
//                (v4.primaryAddress ? `𝙄𝙋𝙫𝟰 : ${v4.primaryAddress} \n` : '') +
...
//                `IP :  ${getFlagEmoji(info.countryCode)} | ${info.query}`, //\n` +
                `𝙄𝙣𝙛𝙤 : ${getFlagEmoji(info.countryCode)}・${info.city}・${info.isp}`,// \n`  +
